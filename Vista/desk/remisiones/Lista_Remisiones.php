...
$obj->Codigo_Remision = $_REQUEST['Codigo_Remision'];
if (empty($obj->Codigo_Remision)) {
    header("location: Remisiones_HistoriaClinica.php?key=".urldecode($llave).'&DocM='.urldecode($DocM));
...
$clas = new conexion();
$conecta = $clas->conectarServidor();
$sql_register = mysqli_query($conecta, "SELECT COUNT(*) AS total_registros from remision where Codigo_Remision LIKE '$obj->Codigo_Remision' ");
...
$clas = new conexion();
$conecta = $clas->conectarServidor();
$query = "SELECT * FROM remision, historiaclinica_remision WHERE Codigo_Remision = '$obj->Codigo_Remision' AND FK_CodigoHistoriaClinica = '$obj->Codigo_HistoriaClinica' GROUP BY '$obj->Codigo_Remision'";
$resultado = mysqli_query($conecta, $query);
$arregloFM = mysqli_fetch_row($resultado);
?>
    ...
    <section class="full-box dashboard-contentPage">
        ...
        <div class="container-fluid">
            <div class="page-header">
                <h1 class="text-titles">Remisiones de <?php echo $arregloHC[2] ?></h1>
                <ul class="breadcrumb breadcrumb-tabs">
                    <li>
                        <a href="Remisiones_HistoriaClinica.php?key=<?php echo urldecode($arregloHC[0]) ?>&DocM=<?php echo urldecode($DocM) ?>" class="btn btn-success">
                            <i class="zmdi zmdi-search"></i> &nbsp; LISTAR REMISIONES
                        </a>
                    </li>
                    <li>
                        <a href="Agregar_Remision.php?key=<?php echo urldecode($arregloHC[0]) ?>&DocM=<?php echo urldecode($DocM) ?>" class="btn btn-info">
        ...
        <!--Panel de busqueda Examenes -->
                    ...
                    <div class="panel panel-success">
                        <div class="panel-heading">
                            <h3 class="panel-title"><i class="zmdi zmdi-search"></i> &nbsp; Lista de Remisiones</h3>
                        </div>
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-hover text-center">
                                    <thead>
                                        <tr>
                                            <th class="text-center">COD REMISION</th>
                                            <th class="text-center">FECHA REMISION</th>
                                    ...
                                    ?>
                                            <tbody>
                                                <tr>
                                                    <td><?php echo $arregloFM[0]; ?></td>
                                                    <td><?php echo $arregloFM[1]; ?></td>
                                                    <td><?php echo $arregloFM[2]; ?></td>
                                                    <td><?php echo $arregloFM[3]; ?></td>
                                                    <td><?php echo $arregloFM[4]; ?></td>
                                                    <td><?php echo $arregloFM[5]; ?></td>
                                                    <td>
                                                        <a href="Modificar_Remision.php?CodHC=<?php echo urldecode($llave) ?>&CodR=<?php echo urldecode($arregloFM[0]) ?>&DocM=<?php echo urldecode($DocM) ?>">
                                                    ...
                                                    <td>
                                                        <a href="Eliminar_Remision.php?CodHC=<?php echo urldecode($llave) ?>&CodR=<?php echo urldecode($arregloFM[0]) ?>&DocM=<?php echo urldecode($DocM) ?>">
                                    ...
                                    <?php
                                        } while ($arregloFM = mysqli_fetch_row($resultado));
                                    }
                                    ?>
                                </table>
                            </div>
