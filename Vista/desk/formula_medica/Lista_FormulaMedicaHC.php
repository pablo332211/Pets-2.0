...
$obj->Codigo_FormulaMedica = $_REQUEST['Codigo_FormulaMedica'];
if (empty($obj->Codigo_FormulaMedica)) {
    header("location: FormulaMedica_HistoriaClinica.php?key=".urldecode($llave).'&DocM='.urldecode($DocM));
}
...
$clas = new conexion();
$conecta = $clas->conectarServidor();
$sql_register = mysqli_query($conecta, "SELECT COUNT(*) as total_registros from formula_medica, historiaclinica_formulamedica WHERE Codigo_FormulaMedica = FK_CodigoFormulaMedica AND FK_CodigoHistoriaClinica  LIKE '$obj->Codigo_HistoriaClinica'");
...
$clas = new conexion();
$conecta = $clas->conectarServidor();
$query = "SELECT * FROM formula_medica WHERE Codigo_FormulaMedica LIKE '$obj->Codigo_FormulaMedica'";
$resultado = mysqli_query($conecta, $query);
