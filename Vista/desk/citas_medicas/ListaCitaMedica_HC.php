...
$obj->FK_CodigoCitaMedica = $_REQUEST['FK_CodigoCitaMedica'];
if (empty($obj->FK_CodigoCitaMedica)) {
    header("location: Lista_AgendarCitaHC.php?key=".urldecode($llave).'&DocM='.urldecode($DocM));
}
...
$clas = new conexion();
$conecta = $clas->conectarServidor();
$sql_register = mysqli_query($conecta, "SELECT COUNT(*) AS total_registros from propietario_citamedica where FK_CodigoCitaMedica LIKE '$obj->FK_CodigoCitaMedica' ");
...
$clas = new conexion();
$conecta = $clas->conectarServidor();
$query = "SELECT * FROM propietario_citamedica WHERE FK_CodigoCitaMedica LIKE '$obj->FK_CodigoCitaMedica'";
$resultado = mysqli_query($conecta, $query);
        ...
        <div class="container-fluid">
            <div class="panel panel-success">
                <div class="panel-heading">
                    <h3 class="panel-title"><i class="zmdi zmdi-format-list-bulleted"></i> &nbsp; LISTA DE CITA</h3>
                </div>
                            ...
                            ?>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="../agendar_cita/Modificar_CitaMedica.php?key=<?php echo urlencode($arreglo[1]) ?>&DocM=<?php echo urlencode($DocM) ?>" class="btn btn-success btn-raised btn-xs">
                                                <i class="zmdi zmdi-check" type="button" name="modificar" value="modificar"></i>
                                            </a>
                                        </td>
                                        <td>
                                            <a href="../agendar_cita/Eliminar_CitaMedica.php?key=<?php echo urlencode($arreglo[1]) ?>&DocM=<?php echo urlencode($DocM) ?>" class="btn btn-danger btn-raised btn-xs">
