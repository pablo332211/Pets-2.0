...
$clas = new conexion();
$conecta = $clas->conectarServidor();
$sql_register = mysqli_query($conecta, "SELECT COUNT(*) AS total_registros from propietario_citamedica GROUP BY FK_DocumentoPropietario");
...
$clas = new conexion();
$conecta = $clas->conectarServidor();
$query = "SELECT pc.FK_CodigoCitaMedica, pc.Fecha_CitaMedica, pc.Observacion_CitaMedica FROM propietario_citamedica pc INNER JOIN mascota_citamedica mc WHERE mc.FK_CodigoMascota = '$arregloHC[1]' AND pc.FK_CodigoCitaMedica= mc.FK_CodigoCitaMedica LIMIT $desde,$pag";
$resultado = mysqli_query($conecta, $query);
    ...
    <section class="full-box dashboard-contentPage">
        ...
        <div class="container-fluid">
            <div class="page-header">
                <h1 class="text-titles">Citas medicas <?php echo $arregloHC[2]; ?></h1>
                <ul class="breadcrumb breadcrumb-tabs">

                    <li>
                        <a href="ListarCitaMedica_HC.php?key=<?php echo urldecode($arregloHC[0]) ?>&DocM=<?php echo urldecode($DocM) ?>" class="btn btn-info">
                            <i class="zmdi zmdi-format-list-bulleted"></i> &nbsp; LISTA DE CITAS MEDICAS
                        </a>
                    </li>
                  <!--  <li>
                        <a href="citas_medicas.php?key=<//?php echo urldecode($arregloHC[0]) ?>&DocM=<//?php echo urldecode($DocM) ?>" class="btn btn-info">
                            <i class="zmdi zmdi-plus"></i> &nbsp; CREAR CITA
                        </a> -->
                    </li>
                </ul>
            </div>
        ...
        <div class="container-fluid">
            <div class="panel panel-info">
                <div class="panel-heading">
                    <h3 class="panel-title"><i class="zmdi zmdi-search"></i> &nbsp; Buscar Cita Medica</h3>
                </div>

                
                <div class="container-fluid">
                    <form method="POST" action="<?php echo "ListaCitaMedica_HC.php?key=".urldecode($llave)?>&DocM<?php echo urldecode($DocM) ?>" class="well">
                        <div class="row">
                            <div class="col-xs-12 col-md-8 col-md-offset-2">
                                <div class="form-group label-floating">
                                    <span class="control-label">Ingrese CODIGO de cita</span>
                                    <input class="form-control" type="text"   name="FK_CodigoCitaMedica" id="FK_CodigoCitaMedica" pattern="[0-9-]{1,20}" required="" />
                                    ...
                                    ?>
                                            <tbody>
                                                ...
                                                <td><?php echo $arreglo[0]; ?></td>
                                                    ...
                                                    <td>
                                                        <a href="../agendar_cita/Modificar_CitaMedicaHC.php?key=<?php echo urldecode($arreglo[0]) ?>&CodHC=<?php echo urldecode($arregloHC[0]) ?>&DocM=<?php echo urldecode($DocM) ?>" class="btn btn-success btn-raised btn-xs">
                                                            <i class="zmdi zmdi-check" type="button" name="modificar" value="modificar"></i>
                                                        </a>
                                                    </td>
                                                    <td>
                                                        <a href="../agendar_cita/Eliminar_CitaMedicaHC.php?key=<?php echo urldecode($arreglo[0]) ?>&CodHC=<?php echo urldecode($arregloHC[0]) ?>&DocM=<?php echo urldecode($DocM) ?>" class="btn btn-danger btn-raised btn-xs">
