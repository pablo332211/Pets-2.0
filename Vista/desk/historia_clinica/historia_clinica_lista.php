...
$obj->Documento_Propietario = $_REQUEST['Documento_Propietario'];
if (empty($obj->Documento_Propietario)) {
    header("location: historia_clinica-buscar.php?" . '&DocM=' . urldecode($DocM));
}
...
$sql_register = mysqli_query($conecta, "SELECT COUNT(*) AS total_registros FROM historia_clinica,propietario,mascota WHERE Documento_Propietario LIKE '$obj->Documento_Propietario' ORDER BY '$obj->Documento_Propietario' ");
...
$clas = new conexion();
$conecta = $clas->conectarServidor();
$query = "SELECT h.Codigo_HistoriaClinica,p.Documento_Propietario,p.NombreA_Propietario,p.ApellidoA_Propietario,m.Nombre_Mascota,m.Especie_Mascota FROM historia_clinica h INNER JOIN mascota m ON h.FK_CodigoMascota = m.Codigo_Mascota INNER JOIN propietario p where p.Documento_Propietario ='$obj->Documento_Propietario' AND m.FK_DocumentoPropietario = '$obj->Documento_Propietario'";
