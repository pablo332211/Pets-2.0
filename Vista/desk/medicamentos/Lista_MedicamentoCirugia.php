...
if (empty($obj->Codigo_Medicamento)) {
    header("location: Lista_Medicamento.php?key=".urldecode($llave) . '&DocM=' . urldecode($DocM) . '&CodC=' . urldecode($CodC));
}
...
$clas = new conexion();
$conecta = $clas->conectarServidor();
$sql_register = mysqli_query($conecta, "SELECT COUNT(*) AS total_registros FROM medicamento m INNER JOIN medicamento_cirugia mc WHERE mc.FK_CodigoCirugia = '$CodC' AND mc.FK_CodigoMedicamento = '$obj->Codigo_Medicamento' GROUP BY '$obj->Codigo_Medicamento'");
