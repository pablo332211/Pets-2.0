...
$obj->Codigo_Medicamento = $_REQUEST['Codigo_Medicamento'];
if (empty($obj->Codigo_Medicamento)) {
    header("location: Lista_Medicamento.php?CodHC=" . urldecode($CodHC) . '&DocM=' . urldecode($DocM) . '&CodFM=' . urldecode($CodFM) );
}
...
$clas = new conexion();
$conecta = $clas->conectarServidor();
$sql_register = mysqli_query($conecta, "SELECT COUNT(*) AS total_registros from medicamento m INNER JOIN formulamedica_medicamento fm WHERE fm.FK_CodigoFormulaMedica = '$CodFM' AND fm.FK_CodigoMedicamento = '$obj->Codigo_Medicamento' GROUP BY '$obj->Codigo_Medicamento'");
...
$clas = new conexion();
$conecta = $clas->conectarServidor();
$query = "SELECT m.Codigo_Medicamento,m.Nombre_Medicamento, m.Presentacion_Medicamento,fm.Dosis_Medicamento,fm.Administracion_Medicamento,fm.Observacion_Medicamento FROM medicamento m INNER JOIN formulamedica_medicamento fm WHERE fm.FK_CodigoFormulaMedica = '$CodFM' AND m.Codigo_Medicamento LIKE '$obj->Codigo_Medicamento' GROUP BY Codigo_Medicamento";
$resultado = mysqli_query($conecta, $query);
            ...
            <div class="container-fluid">
                <div class="panel panel-info">
                    ...
                    <div class="container-fluid">
                        ...
                        <form method="POST" action="<?php echo "Lista_Medicamento.php?CodHC=" . urldecode($CodHC) ?>&DocM<?php echo urldecode($DocM) ?>&CodFM=<?php echo urldecode($CodFM) ?>" class="well">
                            <div class="row">
                                <div class="col-xs-12 col-md-8 col-md-offset-2">
                                    <div class="form-group label-floating">
                                        ...
                                        <input type="hidden" name="CodHC" id="CodHC" value="<?php echo $CodHC; ?>">
                                        <input type="hidden" name="CodFM" id="CodFM" value="<?php echo $CodFM; ?>">
                                    </div>
                                </div>
                                <div class="col-xs-12">
                                    <p class="text-center">
                    ...
                    <div class="panel panel-success">
                        <div class="panel-heading">
                            <h3 class="panel-title"><i class="zmdi zmdi-format-list-bulleted"></i> &nbsp; LISTA MEDICAMENTOS DE FORMULA MEDICA</h3>
                        </div>
                                    ...
                                    ?>
                                            <tbody>
                                                ...
                                                <td><?php echo $arreglo[0]; ?></td>
                                                    ...
                                                    <td>
                                                        <a href="Modificar_Medicamento.php?CodHC=<?php echo urlencode($arregloHC[0]) ?>&CodM=<?php echo urlencode($arreglo[0]) ?>&CodFM=<?php echo urlencode($CodFM) ?>&DocM=<?php echo urlencode($DocM) ?>" class="btn btn-success btn-raised btn-xs">
                                                            <i class="zmdi zmdi-check" type="button" name="modificar" value="modificar"></i>
                                                        </a>
                                                    </td>
                                                    <td>
                                                        <a href="Eliminar_Medicamento.php?CodHC=<?php echo urlencode($arregloHC[0]) ?>&CodM=<?php echo urlencode($arreglo[0]) ?>&CodFM=<?php echo urlencode($CodFM) ?>&DocM=<?php echo urlencode($DocM) ?>" class="btn btn-danger btn-raised btn-xs">
